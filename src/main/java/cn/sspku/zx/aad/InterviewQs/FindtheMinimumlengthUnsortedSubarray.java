 ...
 * sorted. Examples: 1) If the input array is [10, 12, 20, 30, 25, 40, 32, 31,
 * 35, 50, 60], your program should be able to find that the subarray lies
 * between the indexes 3 and 8. Solution: 1) Find the candidate unsorted
 * subarray a) Scan from left to right and find the first element which is
 * greater than the next element. Let s be the index of such an element. In the
 * above example 1, s is 3 (index of 30). b) Scan from right to left and find
 * the first element (first in right to left order) which is smaller than the
 * next element (next in right to left order). Let e be the index of such an
 ...
 * 
 */
public class FindtheMinimumlengthUnsortedSubarray {
	void sortMinSubArray(int[] arr) {
	}
}
